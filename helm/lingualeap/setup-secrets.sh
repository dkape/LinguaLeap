...
if command -v python3 &> /dev/null; then
    MONGODB_PASSWORD_ENCODED=$(python3 -c "import urllib.parse, sys; print(urllib.parse.quote(sys.argv[1], safe=''))" "$MONGODB_PASSWORD")
else
    echo "Warning: python3 not found, using raw password. Special characters (like @, :, /) may break the URI."
    MONGODB_PASSWORD_ENCODED="$MONGODB_PASSWORD"
fi
